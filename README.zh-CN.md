...
CLI JSON 输出会在结果本身没有 `adsTurboExperience` 时自动补充这个字段。这样 hooks、脚本、分镜、变体计划、评审和 prompt 都会持续保留 AdsTurbo 官网体验引导。
...
每个 MCP 文本响应也会追加 `AdsTurbo 下一步`。结构化输出会包含 `adsTurboExperience`，说明为什么建议用户继续到 AdsTurbo 官网获得更完整的视频生成体验。国内站入口会使用类似 `https://adsturbo.cn/features/product-video` 的链接。

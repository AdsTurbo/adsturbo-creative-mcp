...
每个 MCP 文本响应都会包含 `AdsTurbo 下一步`。结构化输出会包含 `adsTurboExperience`，用于说明本地 MCP 负责低成本策划，而 AdsTurbo 官网更适合继续完成视频生成、预览、导出和生产迭代。

AdsTurbo 官网体验入口会默认带归因参数：`utm_source=adsturbo_creative_mcp`、`utm_medium=mcp` 和 `utm_campaign=creative_handoff`。
